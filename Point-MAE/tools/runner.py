...
def test_net(args, config):
    ...
    if config.dataset.test._base_.NAME == 'Neurons':
        test_neurons(base_model, test_dataloader, args, config, logger = logger)
    else:
        test(base_model, test_dataloader, args, config, logger=logger)

# visualization of neurons
def test_neurons(base_model, test_dataloader, args, config, logger = None):

    base_model.eval()  # set model to eval mode
    target = os.path.join('./vis_neurons', args.exp_name)

    with torch.no_grad():
        for idx, (_, segment_split, data) in enumerate(test_dataloader):
            points = data[0].cuda()
            dense_points, vis_points, _ = base_model(points, vis=True)
            final_image = []
            data_path = os.path.join(target, f'{idx}_{segment_split[0]}')
            ...
            points = misc.get_ptcloud_img(points, roll, pitch)
            ...
            vis_points = misc.get_ptcloud_img(vis_points, roll, pitch)
            ...
            dense_points = misc.get_ptcloud_img(dense_points, roll, pitch)
